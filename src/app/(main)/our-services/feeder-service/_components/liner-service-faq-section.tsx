...
export function LinerServiceFAQSection() {
  ...
  return (
    <section ref={ref} className="py-8 md:py-16 px-4 max-w-6xl mx-auto">
      {/* Section Title */}
      <motion.div
        ...
        <Accordion type="single" collapsible className="w-full space-y-3">
          {displayedFaqs.map((faq, index) => (
            ...
            >
              <AccordionItem
                value={`item-${index}`}
                className="border border-slate-200 rounded-none px-6 data-[state=open]:bg-slate-50"
              >
                <AccordionTrigger className="text-left text-lg font-plus-jakarta-sans font-medium text-slate-900 hover:text-slate-700">
                  {faq.question}
                </AccordionTrigger>
      ...
      >
        <Button
          ...
          className="border-slate-300 text-slate-700 rounded-none text-base font-plus-jakarta-sans font-medium hover:bg-slate-50"
        >
