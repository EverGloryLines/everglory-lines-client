...
export const Timeline = ({ data }: { data: TimelineEntry[] }) => {
  ...
  useEffect(() => {
    ...
    // Added data to dependencies to ensure height recalculates if items swap
  }, [ref, data]);
        ...
        {data.map((item, index) => (
          ...
          >
            <div className="sticky flex flex-col md:flex-row z-40 items-center top-40 self-start max-w-xs lg:max-w-sm md:w-full">
              ...
              <h3 className="hidden md:block text-xl md:pl-20 md:text-4xl font-plus-jakarta-sans font-semibold text-neutral-500 dark:text-neutral-500">
                {item.title}
              </h3>
            </div>

            <div className="relative pl-20 pr-4 md:pl-4 w-full">
              <h3 className="md:hidden block text-2xl mb-4 text-left font-plus-jakarta-sans font-semibold text-neutral-500 dark:text-neutral-500">
                {item.title}
              </h3>
              {item.content}{" "}
            </div>
